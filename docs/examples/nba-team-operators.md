...
the {doc}`external-operator proof workflow </operators/external-operator-proof>`,
which is why these demo operators are kept out of the
{doc}`founding community operator cohort </operators/founding-community-operators>`.
```
