...
- Added an "Onboarding and Initial Backers (proof pending)" section to the
  founding community operator documentation, recording that additional
  operators and backers are being onboarded and preparing their own proof
  material.
...
- No organization, identity provider, or partner is named as an implementer or
  ...
  the external-operator proof workflow rather than asserting unverified
  adoption.
...
- The lineage document is design provenance, not an adoption record. It makes no
  ...
  external-operator proof workflow.
