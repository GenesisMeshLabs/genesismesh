...
HTTP_SURFACES: tuple[Surface, ...] = (
    ...
    Surface("POST", "/admin/recognition-treaties", "Issue treaty", "Create a direct-recognition treaty for another sovereign.", "operator", "operator_signed", "Operator signature (privileged tier)", curated=True),
    Surface("POST", "/admin/recognition-treaties/{treaty_id}/revoke", "Revoke treaty", "End a persisted recognition treaty.", "operator", "operator_signed", "Operator signature (privileged tier)"),
...
CLI_SURFACES: tuple[Surface, ...] = (
    ...
    Surface("CLI", "genesis-mesh trust oversight evaluate", "Oversight evaluate", "Run the 8-check deterministic policy engine against a proposed action — automatic / human_approve / block.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust oversight propose", "Oversight propose", "Agent signs a HumanApprovalRequest for a high-stakes action that requires human countersignature.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust oversight approve", "Oversight approve", "Human custodian countersigns a HumanApprovalRequest, producing a DualSignedCommitment.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust oversight verify", "Oversight verify", "Verify both agent and human signatures on a DualSignedCommitment.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust disclose commit", "Disclose commit", "Build and sign a Merkle commitment over an agreement's capability set (reveals count, not members).", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust disclose prove", "Disclose prove", "Generate a Merkle membership proof for one capability without revealing the rest of the set.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust disclose verify", "Disclose verify", "Verify a CapabilityMembershipProof — checks commitment signature, leaf hash, and Merkle root reconstruction.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust disclose nullify", "Disclose nullify", "Issue a signed single-use nullifier for a proof, preventing replay within the validity window.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust consensus vote", "Consensus vote", "Validator casts a signed approve/reject vote on a JustificationProof for K-of-N consensus.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust consensus assemble", "Consensus assemble", "Assemble K-of-N ValidatorVotes into a signed ConsensusProof once the threshold is met.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust consensus verify", "Consensus verify", "Verify the ConsensusProof assembler signature, vote signatures, and threshold count.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust consensus issue-identity", "Consensus issue-identity", "Derive a short-lived EphemeralExecutionIdentity (default 120 s) from a verified ConsensusProof.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust consensus verify-identity", "Consensus verify-identity", "Verify an EphemeralExecutionIdentity for a specific capability and bearer sovereign.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust consensus assess-cascade", "Consensus assess-cascade", "Assess cascade risk on a set of ValidatorVotes — CDS + TCS scoring; exit 1 if CascadeScore exceeds threshold.", "operator", "cli", "Public GET", curated=True),
    Surface("CLI", "genesis-mesh trust risk create", "Risk create", "Create a new signed PeerRiskSignal for a counterparty. Initial signal defaults to 0.5.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust risk update", "Risk update", "Update a PeerRiskSignal from an ExecutionEvidence outcome (EWMA + time decay). Emits RiskAnomaly if sudden drop detected.", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust risk decay", "Risk decay", "Apply time decay to a PeerRiskSignal without a new evidence update (scheduled maintenance).", "operator", "cli", "Operator signature", curated=True),
    Surface("CLI", "genesis-mesh trust risk show", "Risk show", "Display the current PeerRiskSignal state including signal value, update count, and last update time.", "operator", "cli", "Public GET", curated=True),
