...
def test_tamarin_model_proves_all_lemmas() -> None:
    ...
    # tamarin-prover exits 0 even when a lemma is FALSIFIED, and the word
    # "verified" still appears for the lemmas that did pass.  Checking only
    # those two things therefore passes a model with a disproved lemma, so
    # assert explicitly that nothing was falsified or left incomplete.
    assert "falsified" not in out, (
        f"A lemma was FALSIFIED by tamarin-prover.\nstdout:\n{result.stdout}"
    )
